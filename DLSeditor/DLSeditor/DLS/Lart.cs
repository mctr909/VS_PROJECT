...
namespace DLS
{
	unsafe public struct SCONN
	{
		...
		public SCONN(byte* buff)
		{
			mSource = *(UInt16*)buff;
			buff += 2;
			mControl = *(UInt16*)buff;
			buff += 2;
			mDestination = *(UInt16*)buff;
			buff += 2;
			mTransform = *(UInt16*)buff;
			buff += 2;
			mScale = *(Int32*)buff;
		}
		...
		{
			get {
				return (CONN_SRC_TYPE)mSource;
			}
			set {
				mSource = (UInt16)value;
			}
		}
		...
		{
			get {
				return (CONN_SRC_TYPE)mControl;
			}
			set {
				mControl = (UInt16)value;
			}
		}
		...
		{
			get {
				return (CONN_DST_TYPE)mDestination;
			}
			set {
				mDestination = (UInt16)value;
			}
		}
		...
		{
			get {
				return (CONN_TRN_TYPE)((mTransform >> 10) & 0x000F);
			}
			set {
				mTransform |= (UInt16)(((UInt16)value & 0x000F) << 10);
			}
		}
		...
		{
			get {
				return (CONN_TRN_TYPE)((mTransform >> 4) & 0x000F);
			}
			set {
				mTransform |= (UInt16)(((UInt16)value & 0x000F) << 4);
			}
		}
		...
		{
			get {
				return (CONN_TRN_TYPE)(mTransform & 0x000F);
			}
			set {
				mTransform |= (UInt16)((UInt16)value & 0x000F);
			}
		}
		...
		public byte[] Bytes
		{
			get {
				...
				*(UInt16*)pBuff = mSource;
				pBuff += 2;
				*(UInt16*)pBuff = mControl;
				pBuff += 2;
				*(UInt16*)pBuff = mDestination;
				pBuff += 2;
				*(UInt16*)pBuff = mTransform;
				pBuff += 2;
				*(Int32*)pBuff = mScale;
	...
	unsafe public class CART1 : LIST<SCONN>
	{
	...
	unsafe public class CLART
	{
		...
		public CLART(byte* buff, UInt32 termAddr)
		{
			Art = new CART1();
