...
package com.lushprojects.circuitjs1.client;
	...
	void stamp() {
	    if (sim.mDcAnalysisFlag) {
		// when finding DC operating point, replace cap with a 100M resistor
		mCirManager.stampResistor(nodes[0], nodes[1], 1e8);
		curSourceValue = 0;
		return;
	    }
	    ...
	    if (isTrapezoidal())
		compResistance = mCirManager.mDeltaTime/(2*capacitance);
	    else
		compResistance = mCirManager.mDeltaTime/capacitance;
	    mCirManager.stampResistor(nodes[0], nodes[1], compResistance);
	    mCirManager.stampRightSide(nodes[0]);
	    mCirManager.stampRightSide(nodes[1]);
	}
	void startIteration() {
	    if (isTrapezoidal())
	...
	void doStep() {
	    if (sim.mDcAnalysisFlag)
		return;
	    mCirManager.stampCurrentSource(nodes[0], nodes[1], curSourceValue);
 	}
