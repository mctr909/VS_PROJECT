...
public class CustomLogicElm extends ChipElm {
	...
	for (i = 0; i != getPostCount(); i++) {
	    Pin p = pins[i];
	    if (p.output) {
		mCirManager.stampVoltageSource(0, nodes[i+add], p.voltSource);
		if (hasTriState()) {
		    mCirManager.stampNonLinear(nodes[i+add]);
		    mCirManager.stampNonLinear(nodes[i]);
		}
	    }
	}
    }
	...
	for (i = 0; i != getPostCount(); i++) {
		...
		mCirManager.updateVoltageSource(0, nodes[i+add], p.voltSource, p.value ? 5 : 0);
		
		// add resistor for tri-state if necessary
		if (hasTriState())
		    mCirManager.stampResistor(nodes[i+add], nodes[i], highImpedance[i] ? 1e8 : 1e-3);
	    }
	}
    }
