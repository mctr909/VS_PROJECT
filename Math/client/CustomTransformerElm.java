...
//	    width = max(32, abs(yy-y));
	...
	void stamp() {
	    ...
	    CircuitManager.invertMatrix(xformMatrix, coilCount);

	    double ts = isTrapezoidal() ? mCirManager.mDeltaTime/2 : mCirManager.mDeltaTime;
	    for (i = 0; i != coilCount; i++)
		for (j = 0; j != coilCount; j++) {
		    ...
		    int nj = coilNodes[j];
		    if (i == j)
			mCirManager.stampConductance(nodes[ni], nodes[ni+1], xformMatrix[i][i]);
		    else
			mCirManager.stampVCCurrentSource(nodes[ni], nodes[ni+1], nodes[nj], nodes[nj+1], xformMatrix[i][j]);
		}
	    for (i = 0; i != nodeCount; i++)
		mCirManager.stampRightSide(nodes[i]);
	}
	...
	void doStep() {
	    ...
	    for (i = 0; i != coilCount; i++) {
		int n = coilNodes[i];
		mCirManager.stampCurrentSource(nodes[n], nodes[n+1], coilCurSourceValues[i]);
	    }
 	}
