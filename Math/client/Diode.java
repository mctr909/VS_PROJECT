...
class Diode {
	...
	void stamp(int n0, int n1) {
		nodes[0] = n0;
		nodes[1] = n1;
		mCirManager.stampNonLinear(nodes[0]);
	...
	void doStep(double voltdiff) {
		...
		if (mCirManager.mSubIterations > 100) {
			// if we have trouble converging, put a conductance in parallel with the diode.
			// Gradually increase the conductance value for each iteration.
			gmin = Math.exp(-9 * Math.log(10) * (1 - mCirManager.mSubIterations / 3000.));
			if (gmin > .1)
				gmin = .1;
		}
		...
		if (voltdiff >= 0 || zvoltage == 0) {
			...
			mCirManager.stampConductance(nodes[0], nodes[1], geq);
			mCirManager.stampCurrentSource(nodes[0], nodes[1], nc);
		} else {
			// Zener diode
