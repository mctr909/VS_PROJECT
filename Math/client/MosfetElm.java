...
package com.lushprojects.circuitjs1.client;
	...
	void stamp() {
	    mCirManager.stampNonLinear(nodes[1]);
	    mCirManager.stampNonLinear(nodes[2]);
	...
	boolean nonConvergence(double last, double now) {
	    ...
	    if (mCirManager.mSubIterations > 10 && diff < Math.abs(now)*.001)
		return false;
	    // if we're having trouble converging, get more lenient
	    if (mCirManager.mSubIterations > 100 && diff < .01+(mCirManager.mSubIterations-100)*.0001)
		return false;
	    return true;
	}
	...
	void calculate(boolean finished) {
	    ...
	    mCirManager.stampMatrix(nodes[drain],  nodes[drain],  Gds);
	    mCirManager.stampMatrix(nodes[drain],  nodes[source], -Gds-gm); 
	    mCirManager.stampMatrix(nodes[drain],  nodes[gate],   gm);
	    
	    mCirManager.stampMatrix(nodes[source], nodes[drain],  -Gds);
	    mCirManager.stampMatrix(nodes[source], nodes[source], Gds+gm); 
	    mCirManager.stampMatrix(nodes[source], nodes[gate],  -gm);
	    
	    mCirManager.stampRightSide(nodes[drain],  rs);
	    mCirManager.stampRightSide(nodes[source], -rs);
	}
	
	@SuppressWarnings("static-access")
	void getFetInfo(String arr[], String n) {
	    arr[0] = CirSim.LS(((pnp == -1) ? "p-" : "n-") + n);
