...
//	    setupPins();
	...
	void stamp() {
	    ...
	    // voltage source for outputs
            ...
            mCirManager.stampNonLinear(vn2 + mCirManager.mNodeList.size());
            mCirManager.stampVoltageSource(nodes[3], nodes[2], vn2);
	}
...
//        	    volts[i] = lastVolts[i] + sign(volts[i]-lastVolts[i], limitStep);
            ...
            int vn2 = pins[2].voltSource + mCirManager.mNodeList.size();
            if (expr != null) {
        	// calculate output
        	...
        	if (Math.abs(dx) < 1e-6)
        	    dx = sign(dx, 1e-6);
        	mCirManager.stampMatrix(vn2, vn1, -dx);
        	// adjust right side
        	rs -= dx*cur;
        	//                	sim.console("ccedx " + cur + " " + dx + " " + rs);
        	mCirManager.stampRightSide(vn2, rs);
            }
