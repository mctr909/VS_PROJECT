...
class AnalogSwitch2Elm extends AnalogSwitchElm {
	...
	if ((flags & FLAG_INVERT) != 0)
	    open = !open;
	if (open) {
	    mCirManager.stampResistor(nodes[0], nodes[2], r_on);
	    mCirManager.stampResistor(nodes[0], nodes[1], r_off);
	} else {
	    mCirManager.stampResistor(nodes[0], nodes[1], r_on);
	    mCirManager.stampResistor(nodes[0], nodes[2], r_off);
	}
    }
