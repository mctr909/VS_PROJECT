...
class CCCSElm extends VCCSElm {
	...
	void stamp() {
		...
		mCirManager.stampVoltageSource(nodes[0], nodes[1], vn1, 0);

		mCirManager.stampNonLinear(nodes[2]);
		mCirManager.stampNonLinear(nodes[3]);
	}
	...
	void doStep() {
		...
		if (broken) {
			...
			mCirManager.stampResistor(nodes[inputCount], nodes[inputCount + 1], 1e8);
			return;
		}
		...
		int vn1 = pins[1].voltSource + mCirManager.mNodeList.size();
		if (expr != null) {
			// calculate output
			exprState.values[8] = cur; // I = current
			exprState.t = mCirManager.mTime;
			...
			if (Math.abs(dx) < 1e-6)
				dx = sign(dx, 1e-6);
			mCirManager.stampCCCS(nodes[3], nodes[2], pins[1].voltSource, dx);
			// adjust right side
			rs -= dx * cur;
			// sim.console("ccedx " + cur + " " + dx + " " + rs);
			mCirManager.stampCurrentSource(nodes[3], nodes[2], rs);
		}
