...
class CC2Elm extends ChipElm {
	...
	void stamp() {
		// X voltage = Y voltage
		mCirManager.stampVoltageSource(0, nodes[0], pins[0].voltSource);
		mCirManager.stampVCVS(0, nodes[1], 1, pins[0].voltSource);
		// Z current = gain * X current
		mCirManager.stampCCCS(0, nodes[2], pins[0].voltSource, gain);
	}
