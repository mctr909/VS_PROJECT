...
public class AudioOutputElm extends CircuitElm {
	...
	int getNextLabelNum() {
	    int i;
	    int num = 1;
	    if (sim.mElmList == null)
		return 0;
	    for (i = 0; i != sim.mElmList.size(); i++) {
	...
	void getInfo(String arr[]) {
	    ...
	    arr[2] = "start = " + getUnitText(dataFull ? mCirManager.mTime-duration : dataStart, "s");
	...
	void stepFinished() {
	    dataSample += volts[0];
	    dataSampleCount++;
	    if (mCirManager.mTime >= nextDataSample) {
	...
	void setDataCount() {
	    ...
	    dataStart = mCirManager.mTime;
	    dataPtr = 0;
	    dataFull = false;
	    sampleStep = 1./samplingRate;
	    nextDataSample = mCirManager.mTime+sampleStep;
	}
