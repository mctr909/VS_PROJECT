...
class FMElm extends CircuitElm {
	...
	curcount = 0;
     ...
     void stamp() {
	    mCirManager.stampVoltageSource(0, nodes[0], voltSource);
    }
    void doStep() {
	    mCirManager.updateVoltageSource(0, nodes[0], voltSource, getVoltage());
    }
    double getVoltage() {
	double deltaT=mCirManager.mTime-lasttime;
	lasttime=mCirManager.mTime;
	double signalamplitude=Math.sin((2*pi*(mCirManager.mTime-freqTimeZero))*signalfreq);
