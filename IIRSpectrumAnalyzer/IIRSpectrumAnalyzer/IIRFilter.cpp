...
IIRFilter::IIRFilter(int sampleRate, int banks)
{
	...
	for (int i = 0; i < banks; ++i)
	{
		...
		m_banks[i].bDelay1 = 0.0;
		m_banks[i].bDelay2 = 0.0;
	}
}
...
IIRFilter::Lowpass(int bankNo, double freq, double q)
{
	...
	bank->a0 = 1.0 + alpha;
	bank->a1 = -2.0 * cos(omega);
	bank->a2 = 1.0 - alpha;
	bank->b0 = (1.0 - cos(omega)) / 2.0;
	bank->b1 = 1.0 - cos(omega);
	bank->b2 = (1.0 - cos(omega)) / 2.0;
}
...
IIRFilter::Peaking(int bankNo, double freq, double oct, double gain)
{
	...
	bank->a2 = 1.0 - alpha / g;
	bank->b0 = 1.0 + alpha * g;
	bank->b1 = -2.0 * cos(omega);
	bank->b2 = 1.0 - alpha * g;;
}

double
IIRFilter::Exec(int bankNo, double input)
{
